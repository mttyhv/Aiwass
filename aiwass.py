...
# Função para pegar um verso de um livro, dado seu nome, capítulo e versículo
def get_verse(book_title, chapter, verse):
    texts = load_texts()
    book = texts.get(book_title)
    if book:
        return book.get('verses', {}).get(str(chapter), {}).get(str(verse))
    return None
...
# Comando para mostrar um verso específico, dado o livro e capítulo
@bot.tree.command(name='verse', description='Mostra um verso específico')
async def get_verse_command(interaction: discord.Interaction, book_title: str, chapter: str, verse: int):
    verse_text = get_verse(book_title, chapter, verse)
    if verse_text:
        output = f"*{verse_text}*"
        ...
        await interaction.response.send_message(output)
    log_interaction(interaction.user, 'verse', book_title, chapter, verse, output=output)  # Registra o output

# Comando para mostrar um verso aleatório, dado o livro
@bot.tree.command(name='random', description='Mostra um verso aleatório')
async def random_verse(interaction: discord.Interaction, book_title: str):
    texts = load_texts()
    book = texts.get(book_title)
    if book:
        chapter = random.choice(list(book['verses'].keys()))
        verse = random.choice(list(book['verses'][chapter].keys()))
        verse_text = book['verses'][chapter][verse]
        output = f"`{chapter}.{verse}`: *{verse_text}*"
        await interaction.response.send_message(output)
    else:
        output = 'Livro não encontrado.'
        await interaction.response.send_message(output)
    log_interaction(interaction.user, 'random', book_title, output=output)  # Registra o output
...
async def search_verses(interaction: discord.Interaction, book_title: str, keywords: str):
    texts = load_texts()
    if book_title not in texts:
        output = f"O livro '{book_title}' não existe."
        await interaction.response.send_message(output)
        log_interaction(interaction.user, 'search', book_title, keywords, output=output)
        return

    book = texts[book_title]
    results = []
    count = 0  # Contador de aparições
    for chapter, verses in book['verses'].items():
    ...
    if results:
        ...
        for result in results:
            if len(current_message + result) <= message_limit:
                ...
                current_message = result  # Começa uma nova mensagem
...
async def autocomplete_chapter(interaction: discord.Interaction, current: str) -> list[Choice]:
    texts = load_texts()
    focused_option = interaction.data.get('options', [{}, {}])[0].get('value')
    book = texts.get(focused_option)
    if book:
        chapters = [chapter for chapter in book['verses'].keys() if str(current) in chapter]
        return [Choice(name=chapter, value=str(chapter)) for chapter in chapters]
    return []
